...
export default function InfoSectionCard({ id, title, icon, active_icon, description, setIndex, index,}){
        ...
        return(
            <motion.div
                    ...
                    className={`select-card bg-[${(index == id) ? '#FEAD20' : '#323445'}] mobile-s:w-[12rem] mobile-s:h-[12rem] inline-block mx-3 rounded-sm transition ${(index == id) ? 'shadow-none' : 'shadow-2xl'} cursor-pointer duration-0`} onClick={()=> {setIndex(id); setTextColor('white')}}
                    >
